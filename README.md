...
* **Governing Equation:** The underlying physical profile follows single-exponential decay:
  $$\frac{dP_e}{dt} = -\lambda P_e \implies P_e(t) = \exp(-t / T_1)$$
...
* **PINN (Physics-Informed Neural Network):** Parallel SIREN networks that jointly predict the population curve $y(t)$ and the continuous decay rate $\lambda(t)$. It optimizes a combined loss function:
  $$\text{Loss} = e^{-\sigma_d} \text{Loss}_{\text{data}} + \sigma_d + e^{-\sigma_p} \text{Loss}_{\text{physics}} + \sigma_p$$
  where $\sigma_d$ and $\sigma_p$ are learned adaptive log-variance parameters used to dynamically balance data fidelity and the physical derivative constraint ($\frac{dy}{dt} + \lambda y = 0$).
...
2. **Two-Stage Optimization:** Neural networks are trained for 400 epochs using the Adam optimizer ($lr = 1\times10^{-3}$) to discover coarse patterns, followed by a localized L-BFGS optimization phase with a strong Wolfe line search to enforce strict convergence.
...
* **Generalization Gap (Gen):** Quantifies over-fitting tendencies using the normalized difference:
  $$\text{Gap} = \frac{|\text{Train\_MSE} - \text{Test\_MSE}|}{\text{Test\_MSE} + 1\times10^{-8}}$$
...
$$\Delta\text{BIC} = \text{BIC}_{\text{1-component}} - \text{BIC}_{\text{2-component}}$$
